...
export default function BabylonSquare({onSelect}:Props){const canvas=useRef<HTMLCanvasElement>(null);const [status,setStatus]=useState('Loading the Square…');useEffect(()=>{let disposed=false;let cleanup=()=>{};(async()=>{
  ...
  try{
    ...
    topFive.forEach((mesh,index)=>{const [name,tag,bg,fg]=featured[index];mesh.metadata={...(mesh.metadata||{}),rank:index+1};mesh.isPickable=true;
      const tex=new B.DynamicTexture(`featured_${index+1}`,{width:1024,height:512},scene,false);const ctx=tex.getContext();ctx.fillStyle=bg;ctx.fillRect(0,0,1024,512);ctx.strokeStyle=fg;ctx.lineWidth=12;ctx.strokeRect(18,18,988,476);ctx.fillStyle=fg;ctx.font='900 110px Arial';ctx.fillText(`#${index+1}`,48,125);ctx.font='900 130px Arial';ctx.fillText(name,48,286);ctx.font='bold 30px monospace';ctx.fillText(tag,52,382);tex.update();const bm=new B.StandardMaterial(`featured_material_${index+1}`,scene);bm.diffuseTexture=tex;bm.emissiveTexture=tex;bm.emissiveColor=B.Color3.White();bm.disableLighting=true;bm.backFaceCulling=false;mesh.material=bm;
    });
    candidates.slice(5).forEach((mesh,index)=>{const rank=index+6;mesh.metadata={...(mesh.metadata||{}),rank};mesh.isPickable=true;const hue=(rank*37)%360;const tex=new B.DynamicTexture(`ranked_billboard_${rank}`,{width:1024,height:512},scene,false);const ctx=tex.getContext();ctx.fillStyle=`hsl(${hue} 55% 18%)`;ctx.fillRect(0,0,1024,512);ctx.strokeStyle='#d7ff35';ctx.lineWidth=12;ctx.strokeRect(18,18,988,476);ctx.fillStyle='#d7ff35';ctx.font='900 190px Arial';ctx.fillText(`#${rank}`,45,225);ctx.fillStyle='#fff';ctx.font='900 62px Arial';ctx.fillText('TIMESQUARE.LOL',50,350);ctx.font='bold 28px monospace';ctx.fillText('CLICK TO OUTBID THIS BILLBOARD',54,420);tex.update();const mat=new B.StandardMaterial(`ranked_material_${rank}`,scene);mat.diffuseTexture=tex;mat.emissiveTexture=tex;mat.disableLighting=true;mat.backFaceCulling=false;mesh.material=mat;});
