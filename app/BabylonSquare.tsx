...
export default function BabylonSquare({onSelect}:Props){const canvas=useRef<HTMLCanvasElement>(null);useEffect(()=>{let disposed=false;let cleanup=()=>{};(async()=>{
  ...
  const addBillboard=(adIndex:number,x:number,y:number,z:number,w:number,h:number,ry=0)=>{const ad=ads[adIndex];const frame=B.MeshBuilder.CreateBox('frame',{width:w+.7,height:h+.7,depth:.45},scene);frame.position.set(x,y,z);frame.rotation.y=ry;frame.material=mat('frameMat',new B.Color3(.035,.04,.045));
    const plane=B.MeshBuilder.CreatePlane('billboard_'+String(ad.rank).padStart(2,'0'),{width:w,height:h,sideOrientation:B.Mesh.DOUBLESIDE},scene);plane.position.set(x,y,z+.24);plane.rotation.y=Math.PI+ry;plane.metadata={rank:ad.rank};
    ...
    const bm=new B.StandardMaterial('billboardMat'+ad.rank,scene);bm.diffuseTexture=tex;bm.emissiveTexture=tex;bm.emissiveColor=new B.Color3(.75,.75,.75);bm.disableLighting=true;bm.backFaceCulling=false;plane.material=bm;
