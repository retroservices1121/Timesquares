...
import {REAL_BILLBOARD_SLOT} from '../../../lib/real-billboard';
...
export async function POST(request:Request){
  ...
  if(requestedSlot===REAL_BILLBOARD_SLOT)return Response.json({error:'Billboard #49 is a fixed-price Real Times Square package, not a competitive bid.'},{status:400});
  const projected=current.findIndex((value,index)=>index+1!==REAL_BILLBOARD_SLOT&&amount>=minimumBid(value))+1;let slot=mode==='best'?(projected||BILLBOARD_COUNT):requestedSlot;if(slot===REAL_BILLBOARD_SLOT)slot=REAL_BILLBOARD_SLOT+1;const minimum=minimumBid(current[slot-1]);if(amount<minimum)return Response.json({error:`Minimum for Billboard ${slot} is $${minimum.toLocaleString()}.`},{status:400});
