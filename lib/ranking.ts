...
export const rankBids=(bids:RankedBid[])=>[...bids].sort((a,b)=>b.amount-a.amount||a.createdAt.localeCompare(b.createdAt)).map((bid,index)=>({...bid,rank:index+1,billboardId:index<103?`billboard_${String(index+1).padStart(3,'0')}`:null}));
